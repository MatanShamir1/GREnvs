...
class PandaGymWrapper(GoalRecognitionWrapper):
	...
	def __init__(self, env: gymnasium.Env, goal: Optional[PandaGymDesiredGoalList], action_space: Box):
	...
	def _reset_goals(self) -> np.ndarray:
		if self.goal is None:
			goal = self.env.np_random.uniform(
				self._hooked_robot_env.task.goal_range_low, self._hooked_robot_env.task.goal_range_high
			)
			return goal
		else:
			self.goal.reset()
