...
def make_parking_wrapped(**kwargs):
	...
	assert len(set(goal.goal_list) & set(parked_cars)) == 0, f"cant be intersection between {goal.goals} & {parked_cars}"

	assert type(goal) == ParkingGoalList, f"type of goal should be ParkingGoalList not {type(goal)}"
		...
		for goal_index, parked_cars in [
			...
			register(
				...
				kwargs={
					...
					"n_spots": spots_in_row,
					"parked_cars": parked_cars,
					"goal": ParkingGoalList([x for x in range(spots_in_row * 2) if x not in parked_cars])
				},
				order_enforce = False,
				disable_env_checker = True,
			)
